   ...
   */
    ...
    */
    public final static String validateInteger(final Map<String,?> params,final String... fields){
        for(int i = 0; i < fields.length; i++){
            try {
                final String value = (String)params.get(fields[i]);
                if(value == null){
    ...
    */
    public final static PageFormData dataTableMysql(final PageFormData pageFormData) throws Exception{
        ...
        if(column != null && column.length() > 0 && sort != null && sort.length() > 0){
            pageFormData.put("column",ToolString.sqlInject(column));//排序字段 order by name desc
            pageFormData.put("order",ToolString.sqlInject(sort));//排序关键字(升序|降序)
        }
        return pageFormData;
    }
